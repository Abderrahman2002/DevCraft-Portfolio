  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-5 dark:opacity-10">
        <div className="absolute inset-0 grid grid-cols-12 gap-2 w-full h-full">
          {Array.from({ length: 12 }).map((_, colIndex) => (
            <div key={colIndex} className="h-full border-r border-gray-300 dark:border-gray-700"></div>
          ))}
        </div>
        <div className="absolute inset-0 grid grid-rows-12 gap-2 w-full h-full">
          {Array.from({ length: 12 }).map((_, rowIndex) => (
            <div key={rowIndex} className="w-full border-b border-gray-300 dark:border-gray-700"></div>
          ))}
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-6 py-12 md:py-24 relative z-10 flex flex-col md:flex-row items-center">
        <div className="w-full md:w-1/2 pr-0 md:pr-12 mb-12 md:mb-0">
          <motion.div
            ...
            className="mb-4"
          >
            <span className="inline-block py-1 px-3 rounded-full bg-primary-100 dark:bg-primary-900 text-primary-600 dark:text-primary-400 text-sm font-medium mb-6">
          ...
          >
            <a 
              href="#projects" 
              className="inline-flex items-center justify-center px-6 py-3 rounded-lg bg-primary-600 text-white font-medium hover:bg-primary-700 transition-colors duration-200 shadow-lg hover:shadow-xl"
            >
              View My Work
            </a>
            <a 
              href="#contact" 
              className="inline-flex items-center justify-center px-6 py-3 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white font-medium hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-200"
            >
              Contact Me
            </a>
          </motion.div>
        ...
        >
          <div className="relative overflow-hidden rounded-3xl shadow-2xl">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-r from-primary-600/20 to-secondary-500/20 rounded-3xl"></div>
          </div>
      ...
      <motion.div
        ...
        className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex flex-col items-center text-gray-500 dark:text-gray-400"
      >
